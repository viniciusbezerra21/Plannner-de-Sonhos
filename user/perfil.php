...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'editar_perfil') {
  ...
  $fotoPerfil = $usuario_atual['foto_perfil'];
  ...
  if (isset($_FILES['foto']) && $_FILES['foto']['error'] === UPLOAD_ERR_OK) {
    ...
    if (in_array($tipoArquivo, $tiposPermitidos)) {
      ...
      $_SESSION['mensagem_erro'] = 'Tipo de arquivo não permitido. Use apenas JPG, PNG ou GIF.';
    }
  }
...
$sql = "SELECT nome, nome_conj, email, num_telefone, foto_perfil 
        FROM usuario 
        WHERE id = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $usuario_id);
...
// Se não tiver foto, usa iniciais
  ...
  <header class="header">
    <div class="container">
      ...
      <div id="mobileMenu" class="mobile-menu">
        <a href="../index.php" class="nav-link">Início</a>
        <a href="perfil.php" class="nav-link">Perfil</a>
        <a href="../pages/calendar.php" class="nav-link">Agenda</a>
        <a href="../pages/financeiro.php" class="nav-link">Financeiro</a>
      </div>
    </div>
  </header>
      ...
      <div class="feature-detailed-card profile-card">
        <div class="profile-photo">
          <?php if (!empty($fotoPerfil)): ?>
            <img src="uploads/perfil/<?php echo htmlspecialchars($fotoPerfil); ?>" alt="Foto de perfil">
          <?php else: ?>
        ...
        <p><?php echo htmlspecialchars($numTelefone); ?></p>
        <!-- Mudando link para botão que abre modal -->
  ...
  <div id="editModal" style="animation: staticFadeIn 0.3s ease-out;" class="modal">
    <div style="animation: slideInFromBottom 0.3s ease-out forwards;" class="modal-content">
      ...
      <div class="modal-profile-photo" style="text-align: center; margin-bottom: 2rem;">
        ...
        <?php else: ?>
          <div id="modalFotoPlaceholder" class="modal-profile-placeholder"><?php echo $iniciais; ?></div>
        <?php endif; ?>
      ...
      <form method="POST" enctype="multipart/form-data" id="editForm">
        ...
        <div class="form-group">
          ...
          <input type="text" id="modalNome" name="nome" value="<?php echo htmlspecialchars($nome); ?>" required>
        </div>

        <div class="form-group">
          <label for="modalNomeConj">Nome do Cônjuge</label>
          <input type="text" id="modalNomeConj" name="nome_conj" value="<?php echo htmlspecialchars($nomeConj); ?>" required>
        </div>

        <div class="form-group">
          <label for="modalEmail">E-mail</label>
          <input type="email" id="modalEmail" name="email" value="<?php echo htmlspecialchars($email); ?>" required>
        </div>

        <div class="form-group">
          <label for="modalTelefone">Telefone</label>
          <input type="text" id="modalTelefone" name="telefone" value="<?php echo htmlspecialchars($numTelefone); ?>" required>
        </div>
    ...
    <div class="container footer-content">
      ...
      <div class="footer-links">
        ...
        <ul>
          ...
          <li><a href="../pages/calendar.php">Agenda</a></li>
          <li><a href="../pages/financeiro.php">Financeiro</a></li>
        </ul>
      </div>
